<template>
  <div class="bg-gray-300 text-gray-800 antialiased min-h-screen flex flex-col justify-end">
    <div v-on-clickaway="hideMenu" class="bg-gray-400 px-8 py-8 flex justify-center">
      <ul class="absolute -mt-8 space-y-2" style="bottom: 0px">
        <transition v-for="(email,index) in emails" :key="email.id" name="fade">
          <li v-show="visible" class="transform" :class="email.classes">
            <a
              href="#"
              class="bg-white hover:bg-gray-100 w-80 shadow-md border rounded-lg px-4 py-4 flex items-center"
            >
      ...
      <button
        ...
        class="bg-white hover:bg-gray-100 w-80 h-24 shadow-md border rounded-lg px-4 py-4 flex items-center -mt-24"
      >
        <img
          src="https://s3.amazonaws.com/uifaces/faces/twitter/tanveerrao/128.jpg"
          alt="avatar"
